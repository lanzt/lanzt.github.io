...
PERFECTÍSIMO, CONFIRMAMOOOOOOOOOOOOOOOS y ya tenemos un campo para jugar de ahora en adelante (:
...
⚠️❤️⚠️ **ANUNCIO: Vamos a extraer DBS, TABLAS, COLUMNAS y DATA más por enseñanza que por otra cosa, ya que la máquina no requiere eso para seguir, así que si deseas pasar esa fase da [<u>clic acá</u>](#web-sqli-loadfile) y caes directo a lo necesario para continuar con la máquina, saludes y besitos** ⚠️❤️⚠️ 
...
Nos indica que mediante `MySQL` podemos leer archivos **del sistema** usando la función [LOAD_FILE(<archivo>)](https://www.w3resource.com/mysql/string-functions/mysql-load_file-function.php) (no la había usado y tampoco escuchado, así que a explorarla):
...
⚠️❤️⚠️ **ANUNCIO: Si quieres evitar tooooooda la explicación del código y como encontramos el command-injection, sigue [<u>este link</u>](#web-command-injection-revsh)** ⚠️❤️⚠️ 
